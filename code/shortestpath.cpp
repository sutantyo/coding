...
vector<int> bellmanford(vector<vector<int>>,int,int);
...
int main(int argc, char* argv[]){
	...
	vector<vector<int>> input = 
		...
		vector<int> ans = dijkstra(input,3,15);
		for(int i = 1; i <= 15; i++)
			cout << ans[i] << " ";
		cout << endl;

		ans = bellmanford(input,3,15);
		for(int i = 1; i <= 15; i++)
			cout << ans[i] << " ";
		cout << endl;
		...
		ans = dijkstra(input2,1,5);
		for(int i = 1; i <= 5; i++)
			cout << ans[i] << " ";
		cout << endl;

		ans = bellmanford(input2,1,5);
		for(int i = 1; i <= 5; i++)
			cout << ans[i] << " ";
		cout << endl;
...
vector<int> dijkstra(vector<vector<int>> input, int start, int n){

	map< int,set<pair<int,int>> > adj;
	for(auto& x : input){
		adj[x[0]].insert(make_pair(x[1],x[2]));
	}
	...
	while(!pq.empty()){
	  ...
	  marked[u] = true;
		...
		for(auto& v : adj[u]){
			pq.emplace(w+v.second,v.first);
			if (dist[v.first] == -1)
				dist[v.first] = w + v.second;
		  else
				dist[v.first] = min(dist[v.first],w+v.second);
		}
	}
	return dist;
}

vector<int> bellmanford(vector<vector<int>> input, int start, int n){
